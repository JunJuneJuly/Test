  ...
  setup(){
      ...
      const submitAddress = ()=>{
          ...
          if(userAction.value == 0){
              method.value = 'post',url.value = '/shippings';
          }else if(userAction.value == 1){
              method.value = 'put',url.value = `/shippings/${checkedItem.value.id}`;
          }else {
              method.value = 'delete',url.value = `/shippings/${checkedItem.value.id}`;
          }
          if(userAction.value == 0 || userAction.value ==1){
              ...
              // let errMsg='';
              if(!receiverName){
                  errMsg.value = '请输入收货人名称';
              }else if(!receiverMobile || !/\d{11}/.test(receiverMobile)){
                  errMsg.value = '请输入正确格式的手机号';
              }else if(!receiverProvince){
                  errMsg.value = '请选择省份';
              }else if(!receiverCity){
                  errMsg.value = '请选择对应的城市';
              }else if(!receiverAddress || !receiverDistrict){
                  errMsg.value = '请输入收货地址';
              }else if(!/\d{6}/.test(receiverZip)){
                  errMsg.value = '请输入六位邮编';
              }
              if(errMsg.value){
          ...
          //post:添加地址;delete:删除地址；put:更新地址
          if (method.value == 'post'){
            proxy.$axios.post('/shippings',checkedItem.value).then(()=>{
                closeModal();
                getAddressList();
                message.success('操作成功');
            })
          } else if (method.value == 'delete') {
              proxy.$axios.delete(`/shippings/${checkedItem.value.id}`,checkedItem.value.id).then(()=>{
                  closeModal();
                  getAddressList();
                  message.success('操作成功');
              })
          }else{
              proxy.$axios.put(`/shippings/${checkedItem.value.id}`,checkedItem.value).then(()=>{
                  closeModal();
                  getAddressList();
                  message.success('操作成功');
              })
      ...
      const getCartList = ()=>{
          proxy.$axios.get('/carts').then((res)=>{
              ...
              cartList.value = list.filter(item=>item.productSelected);
