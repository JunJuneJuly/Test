    ...
    export default {
      ...
      setup(){
          ...
          let username = computed(()=>{
             return proxy.$store.state.username;
          });
          let cartcount = computed(()=>{
              return proxy.$store.state.cartcount;
          });
          onMounted(()=>{
              getProductList();
          ...
          //下线
          const loginout = ()=>{
              proxy.$axios.post('/user/logout').then(()=>{
                  message.success('账号已成功下线！')
                  cookie.set('userId','',{expires:'-1'});
                  proxy.$store.commit('saveUserName','');
                  proxy.$store.commit('saveCartCount','0');
              })
          };
          //获取账号的购物车列表
          const getCartCount = ()=>{
              proxy.$axios.get('/carts/products/sum').then((res=0)=>{
                  proxy.$store.commit('saveCartCount',res);
              })
          };
          return{
              phoneList,
